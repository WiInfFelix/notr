...
func (*Book) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case book.ForeignKeys[0]: // chapter_book
			values[i] = new(sql.NullInt64)
		case book.ForeignKeys[1]: // todo_book
			values[i] = new(sql.NullInt64)
		default:
	...
	for i := range columns {
		...
		case book.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field chapter_book", value)
			} else if value.Valid {
				b.chapter_book = new(int)
				*b.chapter_book = int(value.Int64)
			}
		case book.ForeignKeys[1]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field todo_book", value)
			} else if value.Valid {
				b.todo_book = new(int)
				*b.todo_book = int(value.Int64)
			}
		default:
