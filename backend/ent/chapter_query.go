...
func (cq *ChapterQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Chapter, error) {
	...
	if query := cq.withBook; query != nil {
		if err := cq.loadBook(ctx, query, nodes,
			func(n *Chapter) { n.Edges.Book = []*Book{} },
			func(n *Chapter, e *Book) { n.Edges.Book = append(n.Edges.Book, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *ChapterQuery) loadBook(ctx context.Context, query *BookQuery, nodes []*Chapter, init func(*Chapter), assign func(*Chapter, *Book)) error {
	...
	query.Where(predicate.Book(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(chapter.BookColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.chapter_book
		if fk == nil {
			return fmt.Errorf(`foreign-key "chapter_book" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "chapter_book" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
