...
func (tq *TodoQuery) QueryBook() *BookQuery {
...
func (tq *TodoQuery) QueryChapter() *ChapterQuery {
	query := (&ChapterClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TodoQuery) QueryPage() *PageQuery {
	query := (&PageClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TodoQuery) Clone() *TodoQuery {
	...
	return &TodoQuery{
		...
		withBook:    tq.withBook.Clone(),
		withChapter: tq.withChapter.Clone(),
		withPage:    tq.withPage.Clone(),
...
func (tq *TodoQuery) WithBook(opts ...func(*BookQuery)) *TodoQuery {
	query := (&BookClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TodoQuery) WithChapter(opts ...func(*ChapterQuery)) *TodoQuery {
	query := (&ChapterClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withChapter = query
	return tq
}
...
func (tq *TodoQuery) WithPage(opts ...func(*PageQuery)) *TodoQuery {
	query := (&PageClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TodoQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Todo, error) {
	...
	if query := tq.withBook; query != nil {
		if err := tq.loadBook(ctx, query, nodes,
			func(n *Todo) { n.Edges.Book = []*Book{} },
			func(n *Todo, e *Book) { n.Edges.Book = append(n.Edges.Book, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withChapter; query != nil {
		if err := tq.loadChapter(ctx, query, nodes,
			func(n *Todo) { n.Edges.Chapter = []*Chapter{} },
			func(n *Todo, e *Chapter) { n.Edges.Chapter = append(n.Edges.Chapter, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withPage; query != nil {
		if err := tq.loadPage(ctx, query, nodes,
			func(n *Todo) { n.Edges.Page = []*Page{} },
			func(n *Todo, e *Page) { n.Edges.Page = append(n.Edges.Page, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TodoQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Todo, init func(*Todo), assign func(*Todo, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(todo.UserColumn), fks...))
	}))
...
func (tq *TodoQuery) loadBook(ctx context.Context, query *BookQuery, nodes []*Todo, init func(*Todo), assign func(*Todo, *Book)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Todo)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Book(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(todo.BookColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.todo_book
		if fk == nil {
			return fmt.Errorf(`foreign-key "todo_book" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "todo_book" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (tq *TodoQuery) loadChapter(ctx context.Context, query *ChapterQuery, nodes []*Todo, init func(*Todo), assign func(*Todo, *Chapter)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Todo)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Chapter(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(todo.ChapterColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.todo_chapter
		if fk == nil {
			return fmt.Errorf(`foreign-key "todo_chapter" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "todo_chapter" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (tq *TodoQuery) loadPage(ctx context.Context, query *PageQuery, nodes []*Todo, init func(*Todo), assign func(*Todo, *Page)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Todo)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Page(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(todo.PageColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.todo_page
		if fk == nil {
			return fmt.Errorf(`foreign-key "todo_page" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "todo_page" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
