...
func (pq *PageQuery) QueryChapter() *ChapterQuery {
...
func (pq *PageQuery) Clone() *PageQuery {
	...
	return &PageQuery{
		...
		withChapter: pq.withChapter.Clone(),
...
func (pq *PageQuery) WithChapter(opts ...func(*ChapterQuery)) *PageQuery {
	query := (&ChapterClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withChapter = query
	return pq
}
...
func (pq *PageQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Page, error) {
	...
	if query := pq.withChapter; query != nil {
		if err := pq.loadChapter(ctx, query, nodes,
			func(n *Page) { n.Edges.Chapter = []*Chapter{} },
			func(n *Page, e *Chapter) { n.Edges.Chapter = append(n.Edges.Chapter, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PageQuery) loadChapter(ctx context.Context, query *ChapterQuery, nodes []*Page, init func(*Page), assign func(*Page, *Chapter)) error {
	...
	query.Where(predicate.Chapter(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(page.ChapterColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.page_chapter
		if fk == nil {
			return fmt.Errorf(`foreign-key "page_chapter" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "page_chapter" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
