...
func (bq *BookQuery) loadUser(ctx context.Context, query *UserQuery, nodes []*Book, init func(*Book), assign func(*Book, *User)) error {
	...
	query.Where(predicate.User(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(book.UserColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.book_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "book_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "book_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
